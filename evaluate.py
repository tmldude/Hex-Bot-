...
class StockFishEvaluate:
    ...
    def __init__(self, board):
        ...
        self.can_castle_kingside = {'white': not board.castle_states['white_king_moved'] and not board.castle_states['KS_rook_w_moved'], 'black': not board.castle_states['black_king_moved'] and not board.castle_states['KS_rook_b_moved']}
        self.can_castle_queenside = {'white': not board.castle_states['white_king_moved'] and not board.castle_states['QS_rook_w_moved'], 'black': not board.castle_states['black_king_moved'] and not board.castle_states['QS_rook_b_moved']}
        self.en_passant = "-" if not board.en_passant_square_fen else self.square_number_to_fen(board.en_passant_square_fen)
    ...
    def fen_to_board(self, fen):
        ...
        hex_digits = [board[rank][file] for rank in range(8) for file in range(8)]
        
        hex_string = '0x' + ''.join(hex_digits)
        integer_value = int(hex_string, 16)

        color_to_move = Board.BLACK if fen_parts[1] == 'w' else Board.WHITE 
        ...
        last_end_square = self.fen_to_square_number(fen_parts[3]) if fen_parts[3] != '-' else None
        can_en_passant = 1 if fen_parts[3] != '-' else 0
        if color_to_move == Board.WHITE:
