...
def simulate_games(num_games=5):
    ...
    for game in range(num_games):
        eval = ChessEvaluator(model_path=MODEL_PATH,
                              model_type=ModelTypes.KERAS,
                              sf_path=SF_PATH)

        engine = ChessEngine(chess.STARTING_BOARD_FEN,
                             dec_type=1,
                             evaluator=eval,
                             depth=1)
    ...
    average_centipawn_loss_per_move_mini = np.load(
        'average_centipawn_loss_per_move_50k_mini.npy')
...
def main():
    ...
    average_centipawn_loss_per_move_mini = np.load(
        'average_centipawn_loss_per_move_50k_mini.npy')
    average_centipawn_loss_per_move_nega = np.load(
        'average_centipawn_loss_per_move_50k_nega.npy')
